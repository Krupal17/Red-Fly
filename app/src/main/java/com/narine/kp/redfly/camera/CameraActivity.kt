package com.narine.kp.redfly.camera
...
class CameraActivity : AppCompatActivity(), View.OnClickListener, OptionView.Callback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val fakeStamp = findViewById<ConstraintLayout>(R.id.fakeStamp)
        val txtDynamic = findViewById<TextView>(R.id.txtDynamic)
...
//        animator.start()
        ...
        Handler().postDelayed(object : Runnable {
            override fun run() {
                txtDynamic.text = (5..6000).random().toString()
                Handler().postDelayed({
                    val bitmap = createWatermarkBitmap(fakeStamp)
                    watermark.setImageBitmap(bitmap)
                }, 100) // Re-run this block after 1000ms
                Handler().postDelayed(this, 1000) // Re-run this block after 1000ms
            }
        }, 1000)
    }

    fun createWatermarkBitmap(view: View): Bitmap {
        // Measure the view to ensure proper dimensions
        view.measure(
        ...
        // Draw the view onto the canvas of the bitmap
